...
outputSummary <- get(load("../outputCompiled/outputSummary.RData"))
...
df <- outputSummary %>%
    ...
    mutate(group = paste0(simID, mgmtAreaID))
    


png(filename= paste0("pools_Summary.png"),
    width = 7, height = 20, units = "in", res = 600, pointsize=10)

ggplot(df, aes(x = Time, y = value*unitConvFact, group = group,
              #linetype = tenure,
              colour = mgmtAreaName)) +
    theme_dark() +
    facet_grid(mgmtAreaName  ~ variable  ) +
    #facet_wrap(~ variable  ) +
    geom_line() +
